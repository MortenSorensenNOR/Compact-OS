...
struct virtio_virtq* virtq_init(unsigned index) {
    ...
    // 5. Notify the device about the queue size by writing the size of QueueNum
...
// Read/write from/to virtio-blk device
void read_write_disk(void* buf, unsigned sector, int is_write) {
    ...
    if (blk_req->status != 0) {
        printf("virtio: warn: failed to read/write secotr=%d status=%d",
...
void fs_flush(void) {
    ...
    for (int file_i = 0; file_i < FILES_MAX; file_i++) {
        ...
        // Turn the file size into an ocatal string
...
struct process* create_process(const void* image, size_t image_size) {
    ...
    *--sp = (uint32_t) user_entry;  // ra
    ...
    // Map virtio-blk MMIO region to page table so that kernel can access the MMIO registers
...
void yield(void) {
    ...
    // If there's no runnable process other than the currently running one, return and continue execution
